...
\begin{enumerate}
	...
	&=(2+3i)-2i+3=5+1 &\\
	&\sqrt{25}+1 = \abs{z1} &\\
	&  \text{arg}(z_1)=\text{arctan} \left( \frac{\text{Im }z_1}{\text{Re }z_1} \right) = \text{arctan} \left(\frac{1}{5}\right) &
	\end{flalign*}
	\item 
	\begin{flalign*}
	& \left(\frac{1}{1+i}\right)^2 - \frac{1-i}{1+i} &\\
	& z_2=\left(\frac{1}{1+i}\right)^2-\frac{1-i}{1+i} =-\frac{1}{2}i & \\
	& \overset{\text{neutrale Erweiterung}}{=} \left[\frac{1(1-i)}{(1+i)(1-i)}\right] - \frac{(1-i)(1-i)}{1+i(1-i)} & \\
	& = \left[\frac{1-i}{1-i^2}\right]^2-\frac{1-2i+i^2}{1-i^2} = \frac{1}{2}i & \\
	& \abs{z_2} = \sqrt{\left(\frac{1}{2}\right)^2} &\\
	...
	& =(-1-i)(-1-2i+1)^{1004} & \\
	& = (-1-i)(-2i)^{1004} & \\
	& =(-1-i)\cdot 2^{1004}\cdot i^{1004} & \\
	& = -2^{1004}\cdot i ^{1004}& \\
	& \text{Bemerkung: }i ^{1004} = 1 &\\
	& \abs{z_3} = \sqrt{(2^{1004})^2+(2^{1004)})^2}= & \\
	& \sqrt{2(2^{1004})^2} = \sqrt{2}\cdot 2^{1004} & \\
	& \text{arg} z_3 = \frac{5\pi}{4}&
	\end{flalign*}
	\item 
	\begin{flalign*}
	& z_4=\frac{1+i\alpha}{1-i\alpha} & \\
	& \overset{\text{neutrale Erweiterung}}{=} \frac{(1+i\alpha)^2}{1+\alpha^2} = \frac{1+2i\alpha-\alpha^2}{1+\alpha^2} & \\
	& \frac{\sqrt{1+2\alpha^2+\alpha^4}}{1+\alpha^2} = \frac{\sqrt{(1+\alpha^4)^2}}{1+\alpha^2} = 1 & \\
	& \text{arg}(z_4) = 
	\begin{cases} 
	0, & \alpha = 0 \\ 
	1 \frac{\pi}{2}, & \alpha = \pm 1 \\
	\text{arctan}\left(\frac{2\alpha}{1-\alpha^2}\right), & \alpha \in (-1,0) \text{ oder } 0,1
	\end{cases}
	\end{flalign*}
...
\newpage
\section{Übungsblatt 1, Aufgabe 2: 4 Punkte, \GruppeA}
\begin{enumerate}
	\item 
	\begin{flalign*}
	& M_1:=\{z;\abs{z-a} \geq r, a\in C, r > 0\} &
	\end{flalign*}
	\begin{tikzpicture}[x=.5cm, y=.5cm]
	% Achsen zeichnen
	\draw[->,thick] (-1,0) -- (6,0) node[right] {Re};
	\draw[->,thick] (0,-1) -- (0,6) node[above] {Im};
	% Achsen beschriften
	% Funktionen zeichnen:
	\clip(-1,-1) rectangle (10,10);
	\draw (4,4) circle (2);
	\draw (4,3.5) node[anchor=south] {z};
	\draw (6,6) node[anchor=south] {$M_1$};
	\draw (4,4) -- node[above] {r} (6,4);
	\end{tikzpicture} \\
	Die Menge $M_1$ ist alles \enquote{außerhalb des Kreises}.
	\item 
	\begin{flalign*}
	& M_2:=\{z;\text{Re} \frac{1}{z}=\frac{1}{2}\} & \\
	& \frac{1}{z} = \frac{\bar{z}}{\abs{z}^2} = \frac{x-iy}{x^2+y^2} &\\
	& \Rightarrow \text{Re} \frac{1}{z} = \frac{x}{x^2+y^2} = \frac{1}{2} \Rightarrow 2x = x^2+y^2 & \\
	& 0 = x^2-2x + y^2 \Leftrightarrow 1 = (x^2 - 2x+1)+y^2=(x-1)^2+y^2
	\end{flalign*}
	\begin{tikzpicture}[
	pfeil/.style={thick,<-,shorten >=2pt,shorten <=2pt,>=stealth},
	x=.5cm, y=.5cm]
	%Raster zeichnen
%	\foreach \x in {-1,0,1,2,5}
	\draw (2,-.1) -- (2,.1) node[below=4pt] {1};
%	\foreach \y in {-1,0,1,2,5}
%	\draw (-.1,\y) -- (.1,\y) node[left=4pt] {$\scriptstyle\y$};
	% Achsen zeichnen
	\draw[->,thick] (-1,0) -- (5,0) node[right] {Re};
	\draw[->,thick] (0,-1) -- (0,3) node[above] {Im};
	% Achsen beschriften
	%Funktionen zeichnen:
	\clip(-2.5,-2.5) rectangle (8,6);
	\draw (2,0) circle (2);
	\draw (6,3) node[anchor=south] {$M_2$};
	\draw [pfeil] (2,2) -- (6,3);
	\end{tikzpicture} \\
	Die Menge $M_2$ ist alles auf dem Kreis.
	\item
	\begin{flalign*}
	& M_3 := \{z; \abs{\frac{z-1}{z}} \} & \\
	& z = x - iy:\frac{z-1}{z} = \frac{(z-1)\bar{z}}{\abs{\bar{z}}^2} = \frac{\abs{z}^2-\bar{z}}{\abs{z}^2} = \frac{x^2+y^2-x+iy}{x^2+y^2} & \\
	& \Rightarrow \abs{\frac{z-1}{z}} = \sqrt{\frac{((x^2+y^2)-x^2)+y^2}{x^2+y^2}}<1& \\
	& \Leftrightarrow \frac{((x^2+y^2)-x)^2+y^2}{(x^2+y^2)^2} < 1 & \\
	& \Leftrightarrow ((x^2+y^2)^2-x)^2+y^2<(x^2+y^2) & \\
	& \Leftrightarrow ((x^2+y^2)^2-x)^2+y^2 < (x^2+y^2)& \\
	& \text{Bemerkung: } ((x^2+y^2)^2-x)^2 = (x^2+y^2)^2 - 2x(x^2+y^2)+(x^2+y^2) &\\
	& \Leftrightarrow (x^2+y^2)-2x(x^2+y^2) = (1-2x)(x^2+y^2)<0 & \\
	& \text{Bemerkung: } (x^2+y^2) \geq 0 &\\
	...
	%Raster zeichnen
	%	\foreach \x in {-1,0,1,2,5}
	\draw (2,-.1) -- (2,.1) node[below right=2pt] {$\frac{1}{2}$};
	%	\foreach \y in {-1,0,1,2,5}
	%	\draw (-.1,\y) -- (.1,\y) node[left=4pt] {$\scriptstyle\y$};
	% Achsen zeichnen
	\draw[->,thick] (-2,0) -- (3,0) node[right] {Re};
	\draw[->,thick] (0,-2) -- (0,3) node[above] {Im};
	% Achsen beschriften
	%Funktionen zeichnen:
	\clip(-2.5,-2.5) rectangle (8,6);
	\draw (2,-2) -- (2,3);
	\draw (3,2) node[anchor=south] {$M_3$};
	\end{tikzpicture} \\
	Die Menge $M_3$ ist alles \enquote{rechts} von der Linie.
\end{enumerate}

\newpage
\section{Übungsblatt 1, Aufgabe 3: 4 Punkte, \GruppeA}
\begin{flalign*}
& \text{Sei } p \text{ reelles Polynom } z_0 \in C. & \\
& \text{Behauptung: } p(z_0)=0 \Leftrightarrow p(\overline{z_0})=0 & \\
& \text{Behauptung: } \overline{p(z)} = \overline{p(\overline{z})} & \\
& \text{Beweis: } \overline{p(z)} = \sum_{k=0}^{n}a_kz^k = \sum_{k=0}^{n}\overline{a_kz^k} & \\
& \sum_{k=0}^{n}\overline{a_k}\cdot\overline{z^k} = \sum_{k=0}^{n} a_kz^{-k} = p(\overline{z}) & Behauptung ~~ \square &\\
& \overline{a_k} \text{reel} = a_k & \\
& \text{Bemerkung: } \overline{z+w} = \overline{z} + \overline{w}, \overline{zw} = \overline{z}\cdot\overline{w} &
\end{flalign*}

\newpage
\section{Übungsblatt 1, Aufgabe 4: 4 Punkte, \GruppeA}

Für $(\abs{a}, \abs{b} < 1)$, zeige man: 
\begin{flalign*}
	& \frac{\abs{a-b}}{\abs{1-\overline{a}b}} < 1  \Leftrightarrow \frac{\abs{a-b}}{1-\overline{a}b}^2 < 1 & \\
	& \text{Bemerkung: relle Zahl} <1 \Rightarrow \alpha(\text{reelle Zahl})^2 < 1 &\\
	& \frac{(a-b)(\overline{a}-\overline{b})}{\abs{1-\overline{a}b}^2} = \frac{\abs{a}^2-ab-\overline{a}b+\abs{b}^2}{\abs{1-\overline{a}b}^2} & \\ 
	& = \frac{1 + \abs{a}^2\abs{b}^2-ab-\overline{a}b}{\abs{1-\overline{a}b}^2} -
	\frac{1-\abs{a}^2-\abs{b}^2+\abs{a}^2\cdot\abs{b}^2}{\abs{1+\overline{a}b}^2} & \\
	& = \frac{1-(1-\abs{a}^2)(1-\abs{b}^2)}{\abs{1+\overline{a}b}^2} < 1& \\
	& \text{Bemerkung: Divisor des linken Terms ist größer 0.}
\end{flalign*}
\appendices
	\chapter{}
	...
	\section{Übungsblatt 4}\label{uebungsblatt4}
	\includepdf[pages=-]{blatt4}
\end{document}